...
> 学习目标：掌握如何设计高效的提示词，让大模型输出准确、符合预期的结果。

## 1. 什么是提示词工程（Prompt Engineering）

**定义**：设计和优化输入给大模型的提示词（Prompt），以引导模型产生更符合你需求、高质量输出的过程。

- **提示词（Prompt）：** 输入给 AI 模型的指令、问题、或文本输入
- **工程（Engineering）：** 设计、优化和改进你的输入文本的过程

**核心公式**：
好的 Prompt = 清晰的角色 + 具体的任务 + 明确的约束 + 必要的示例
...
![](F:\githubMd\images\2026-05-29-08-59-47-image.png)
...
把最重要的指令放在提示词的**开头或结尾**，中间位置的内容在长上下文中容易被模型"忽视"——这是大模型的已知特性，称为 **"迷失在中间（Lost in the Middle）"现象**。

技巧：[](https://www.runoob.com/ai-agent/ai-agent-llm.html) [大语言模型基础](https://www.runoob.com/ai-agent/ai-agent-llm.html "大语言模型基础")

[Token (词元)](https://www.runoob.com/ai-agent/token-intro.html "Token (词元)") [](https://www.runoob.com/ai-agent/token-intro.html)
...
![](https://www.runoob.com/wp-content/uploads/2025/12/f481f8cb-0023-4271-9486-e3c2c5a09afb.webp)
...
提示词工程Prompt Engineering清晰指令降低歧义角色设定激活专业知识XML 标签隔离数据与指令思维链先推理后作答防幻觉设计提升可信度少样本示例用例子代替描述提示词链分步链式传递元提示让 AI 优化提示词本文技巧概览
...
提示词工具及案例： [AI 对话提示词(prompt) | 菜鸟工具](https://www.jyshare.com/front-end/9127/)。
...
GPT-3.54K tokensGPT-4 Turbo128K tokensClaude 3.7200K tokensSystem Prompt对话历史当前输入AI 输出所有内容共享同一个上下文窗口，总量不能超过模型上限
...
一定要让 AI **先分析，再下结论**。如果先让它说结论，再让它解释，它会反过来为已有结论找理由——而不是真正在推理。顺序非常关键。
...
![](F:\githubMd\images\2026-05-29-09-23-54-image.png)
...
![](F:\githubMd\images\2026-05-29-09-25-17-image.png)
...
**定义**：不提供任何示例，直接给模型下达指令。

**适用场景**：简单、常见的任务，模型已有充分理解。

**优点**：简单直接，节省 token。  

**缺点**：对复杂或需要特定格式的任务效果不稳定。
...
**定义**：在 Prompt 中提供几个输入输出示例，让模型“照葫芦画瓢”。

**适用场景**：需要特定输出格式、风格，或模型不熟悉的特殊任务。

**关键技巧**：提供 3-5 个覆盖不同情况的示例，且格式必须完全一致。
...
### 2.3 Chain-of-Thought (CoT) (链式思考)

**定义**：引导模型“先思考，后回答”，在给出最终答案前，先输出一步步的推理过程。

**适用场景**：数学题、逻辑推理、需要多步分析的任务。

**优点**：显著提升复杂推理任务的准确率，并且推理过程可追溯。
...
思考：一开始有10个苹果。给了小红3个，剩下 10-3=7个。又买了5个，现在有 7+5=12个。
答案：12个。
```

### 2.4 角色扮演

**定义**：为模型设定一个具体的身份、背景和风格，让它在这个角色框架内回答问题。当你给它设定一个角色时，相当于**激活**了它在那个领域积累的知识模式。

**适用场景**：需要特定语气、专业知识或场景模拟的对话。
...
```text
你是一位有20年经验的技术面试官。请根据以下候选人的自我介绍，提出两个考察其项目深度的技术问题。
```

### 2.5 结构化 Prompt

**定义**：使用 Markdown、XML 或 JSON 等标记，将指令组织成层次分明的区块。

**适用场景**：包含角色、任务、规则、示例等多个部分的复杂指令。
...
**定义**：CoT 的升级版。它让模型同时探索多条不同的推理路径，并自我评估每条路径的可行性，最终选择最优解。

**适用场景**：需要创造性、规划或探索多种可能性的复杂问题（如24点游戏、创意写作）。

**对比**：CoT 是一条路走到黑，ToT 是走一步，看三步，探索多个岔路口。

### 3.2 ReAct (Reasoning + Acting)

**定义**：让模型在推理（Reasoning）的同时，还能执行动作（Acting），如调用工具、查询知识库，并根据观察到的结果（Observation）继续推理。

**适用场景**：需要调用外部工具、获取实时信息、与环境交互的 Agent 任务。

**示例流程**：

```text
Thought: 我需要知道北京今天的天气才能回答。
Action: search_weather(city="北京")
Observation: 北京今天晴，25°C
Thought: 我已经知道了天气，可以回答用户了。
Answer: 今天北京天气晴朗，温度25°C。
```

### 3.3 输出格式约束

**定义**：通过明确的指令，精确控制模型输出的数据结构。

**常用方法**：

| 方法          | 示例                                      |
| ----------- | --------------------------------------- |
| **JSON 约束** | “只输出JSON格式：{"name": "", "summary": ""}” |
| **关键词约束**   | “只回答‘正面’或‘负面’”                          |
| **长度约束**    | “用一句话总结，不超过20个字”                        |

**最佳实践**：在指令中提供一个期望的输出格式示例，效果远超纯文字描述。

## 4. Prompt 优化原则

### 4.1 DO（推荐做法）

- **明确具体**：“将以下标题翻译成英文” 优于 “翻译一下”

- **给出示例**：复杂格式用 Few-shot

- **分解任务**：将复杂任务拆解为多个步骤

- **迭代优化**：好 Prompt 都是改出来的

### 4.2 DON'T（避免做法）

- **模糊不清**：“写一段好的文案” 是不合格的指令

- **过于冗长**：避免在单个 Prompt 中包含冲突或无关信息

- **指令冲突**：不要在同一个 Prompt 里要求“详细说明”又要求“一句话总结”

## 5. 面试常见问题

**Q1: 如何让模型稳定输出 JSON 格式？**

> 我会使用三种方法组合：1. 在 Prompt 中给出期望的 JSON 示例；2. 用结构化 Prompt 明确字段说明；3. 在代码中增加后处理校验，如果解析失败，则触发重新生成或使用备用逻辑。

**Q2: CoT 和 Few-shot 可以一起用吗？**

> 可以，而且效果通常更好。在 Few-shot 中提供的示例本身就是带有推理过程的，模型会同时学习到“输出格式”和“推理逻辑”两个模式。

**Q3: 模型总是“记不住”前面的对话内容怎么办？**

> 这主要是上下文窗口的限制问题。我的解决方案是：1. 对重要信息进行**摘要压缩**；2. 在长对话中启用**滑动窗口**，只保留最近几轮；3. 对于必须长期记忆的关键事实，可以通过外部数据库或变量显式存储。

## 6. 快速记忆表

| 技巧            | 一句话定义  | 适用场景          |
| ------------- | ------ | ------------- |
| **Zero-shot** | 直接提问   | 简单、常见任务       |
| **Few-shot**  | 给几个例子  | 需要格式或风格约束     |
| **CoT**       | 一步步推理  | 数学、逻辑题        |
| **角色扮演**      | 设定身份   | 场景化对话         |
| **结构化**       | 分块组织   | 包含多个部分的复杂指令   |
| **ToT**       | 探索多条路径 | 创造性、规划性问题     |
| **ReAct**     | 推理+行动  | 需要调用工具的 Agent |
| **输出约束**      | 指定格式   | 需要程序化解析结果     |
